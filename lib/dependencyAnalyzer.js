...
function analyzeDependencies(filePath) {
  const result = processPath(
    ...
    (results) => ({
      "Total de dependências": results.reduce((sum, r) => sum + r["Total de dependências"], 0),
      "Dependências externas": results.reduce((sum, r) => sum + r["Dependências externas"], 0),
      "Dependências locais": results.reduce((sum, r) => sum + r["Dependências locais"], 0)
    })
  );

  console.log('DEBUG: analyzeDependencies result =', result); // Depuração
  return result;
}
